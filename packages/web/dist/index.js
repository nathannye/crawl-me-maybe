	...
	const automappedValues = Object.keys(fieldMappings).reduce(
		(acc, key) => {
			if (shouldAutomap(automapSettings, key)) {
				acc[key] =
					baseSeoObject[fieldMappings[key]] || extra[fieldMappings[key]];
			}
			return acc;
		},
		{
...
function buildArticle({ seo, schemaDefaults, extra }) {
	...
	return {
		...
		datePublished: formatSchemaDate(
			autoMap.dates !== false
				? extra?._createdAt || extra?.datePublished
				: extra?.datePublished,
		),
		dateModified: formatSchemaDate(
			autoMap.dates !== false
				? extra?._updatedAt || extra?.dateModified
				: extra?.dateModified,
		),
		author: authorSchema,
...
function buildProduct({ seo, schemaDefaults, extra }) {
	...
	return {
		...
		image,
		brand: buildOrgSchema(brand, true, seo.canonicalUrl),
		// Use reference
		sku: extra?.sku,
		mpn: extra?.mpn,
		gtin: extra?.gtin,
...
function buildEvent({ seo, schemaDefaults, extra }) {
	...
	const organizerSchema = Array.isArray(organizer)
		? organizer
				...
				.filter(Boolean)
		: buildPersonOrOrg(organizer, true, seo.canonicalUrl);
...
function composeSchema({
	...
	const addOrgWithDepartments = (org) => {
		...
		addEntity(orgWithDefaults, (o) =>
			buildOrganization(o, schemaDefaults, baseUrl),
		);
		...
		for (const contributor of extra.contributor) {
			if (
				...
				addEntity(contributor, (person) =>
					buildPersonOrOrg(person, false, baseUrl),
				);
			}
		}
	}
		...
		for (const organizer of extra.organizer) {
			if (organizer && typeof organizer === "object" && "name" in organizer) {
				...
				addEntity(entityWithDefaults, (entity) =>
					buildPersonOrOrg(entity, false, baseUrl),
				);
			}
		}
	}

...
`;
  ...
  const createSitemap = async (filename, urls) => {
    const processedUrls = locales && locales.length > 0 ? generateLocalizedEntries(urls, locales, domain, localeMode) : urls.map((u) => ({ ...u, url: domain + u.url }));
  ...
  return {
    ...
    apply: "build",
    async closeBundle() {
      const outDir2 = path.resolve(process.cwd(), config?.outDir || "dist");
